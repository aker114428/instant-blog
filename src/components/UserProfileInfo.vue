    ...
    box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19); text-align: center;">
        ...
        <div class="user-actions">
            <div v-if="is_me">
                ...
                <a-modal v-model:visible="visible" title="上传头像或修改简介" :ok-text="isEditing ? '保存' : '确定'"
...
export default {
    ...
    components: { UserAddOutlined, TeamOutlined, FormOutlined, UserDeleteOutlined, uploadPhoto },
    props: {
        user: {
            type: Object,
            required: true
    ...
    setup(props, context) {
        ...
        const handleSave = () => {
            if (isEditing.value) {
                ...
                $.ajax({
                    ...
                    data: {
                        selfIntroduction: editedUser.description,
                        userPhoto: editedUser.photo
                    },
                    success(resp) {
                        if (resp.code == 0) {
        ...
        return { handleUpdate, follow, unfollow, is_me, showEditbox, handleSave, handleCancel, visible, isEditing, editedUser };//, editedUserRefs };
    }
}
</script>
