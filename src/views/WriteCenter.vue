...
//文章信息
let text = ref('');
let articleTitle = ref('标题');
let introduction = ref('');
let polishedText = ref('');
let imgUrl = ref('https://source-warehouse.oss-cn-guangzhou.aliyuncs.com/img/默认图片.webp');
let imgUrlStyle = ref(" background-image: url(" + imgUrl.value + ")");
...
function publishArticle() {
    ...
    fetch(backendIP+API_ROUTES.creatArticle, requestOptions)
        ...
        .catch(error => console.log('上传文章失败', error));
}
//文章润色
...
function articleCompletion(){
    if (!text.value) {
        ...
        formdata.append("gpt_message", "补充续写一下给定文章，内容更完整。注意文章要采用markdown格式书写, 返回的文章要分点，排版要美观，文章内容如下：" + text.value);
...
<template>
    ...
    <main class="layout">
        ...
        <div class="submit-modal">
            <a-modal v-model:visible="ivisible" title="修改封图" @ok="handleOk">
                <template #footer>
                    <a-button key="back" @click="handleCancel">取消</a-button>
